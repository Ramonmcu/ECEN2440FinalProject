...
#define SPIDIV     0x04                         // 24 MHz/3 gives ~125 ns for each on bit in byte
...
int run_matrix(void)
{
  ...
  while(1){
      //sendPixel(0xFF, 0x00, 0x00);           // green
      showColor(PIXELS, 0x55, 0x00, 0x00);     // red
      __delay_cycles(12000000);
      showColor(PIXELS, 0x00, 0x55, 0x00);     // red
      __delay_cycles(12000000);
//      __delay_cycles(12000000);
//      sendPixel(0x00, 0x64, 0x00);           // green
//      sendPixel(0x00, 0x00, 0x64);           // blue
//      sendPixel(0x66, 0x66, 0x66);           // white
...
//      show();
//      __delay_cycles(12000000);
//     showColor(PIXELS, 0x00, 0x00, 0x00);  // red
//      __delay_cycles(12000000);
//      showColor(PIXELS, 0x00, 0x00, 0xBB);          // blue
//      __delay_cycles(12000000);
//      showColor(PIXELS, 0x00, 0xBB, 0x00);          // green
//      __delay_cycles(12000000);
  }
}
...
void sendByte (unsigned char b){
    ...
    for (bit = 0; bit < 8; bit++){
      if (b & 0x80) // is MSB set?
          EUSCI_B0->TXBUF = SPILONG;     // long on bit (~700 ns) defined for each clock speed
      else
          EUSCI_B0->TXBUF = SPISHORT;    // short on bit (~350 ns) defined for each clock speed
...
// latch the colors
void show(){
    __delay_cycles(432);            // > ~9 micro seconds
}
